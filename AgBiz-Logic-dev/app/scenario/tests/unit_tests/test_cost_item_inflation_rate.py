...
from scenario.views import CostItemInflationRateViewSet
import copy
...
class CostItemInflationRateAPITestCase(APITestCase):
    """ Test suite for the CostItemInflationRate model REST resource.
    """
    ...
    def setUp(self):
        ...
        self.cost_item = CostItem.objects.create(parent_budget=self.budget, **COST_ITEM_OUTLINE)
    ...
    def test_create(self):
        ...
        new_inflation_rate = copy.deepcopy(COST_ITEM_INFLATION_RATE_OUTLINE)
        new_inflation_rate.update({
            'cost_item': self.cost_item.pk,
            'plan_budget': self.plan_budget.pk,
        })

        viewset = CostItemInflationRateViewSet.as_view({'post': "create"})
        request = self.factory.post('', new_inflation_rate, format='json')
        force_authenticate(request, user=self.user)
        response = viewset(request)

        self.assertEqual(201, response.status_code)
        self.assertEqual(1, len(CostItemInflationRate.objects.all()))
    ...
    def test_retrieve(self):
        ...
        cost_item_inflation_rate = CostItemInflationRate.objects.create(
            plan_budget=self.plan_budget,
            cost_item=self.cost_item,
            **COST_ITEM_INFLATION_RATE_OUTLINE
        )

        viewset = CostItemInflationRateViewSet.as_view({'get': "retrieve"})
        request = self.factory.get('', format='json')
        force_authenticate(request, user=self.user)
        response = viewset(request, pk=cost_item_inflation_rate.pk)

        self.assertEqual(200, response.status_code)
        self.assertEqual(COST_ITEM_INFLATION_RATE_OUTLINE['inflation_rate'], response.data['inflation_rate'])
    ...
    def test_update(self):
        ...
        updated_cost_item_inflation_rate = copy.deepcopy(COST_ITEM_INFLATION_RATE_OUTLINE)
        updated_cost_item_inflation_rate.update({
            'cost_item': self.cost_item.pk,
            'plan_budget': self.plan_budget.pk,
            'inflation_rate': new_inflation_rate
        })

        viewset = CostItemInflationRateViewSet.as_view({'put': "update"})
        request = self.factory.put('', updated_cost_item_inflation_rate, format='json')
        force_authenticate(request, user=self.user)
        response = viewset(request, pk=cost_item_inflation_rate.pk)

        self.assertEqual(200, response.status_code)
        self.assertEqual(str(new_inflation_rate), response.data['inflation_rate'])
    ...
    def test_destroy(self):
        ...
        viewset = CostItemInflationRateViewSet.as_view({'delete': "destroy"})
        request = self.factory.delete('', format='json')
        force_authenticate(request, user=self.user)
        response = viewset(request, pk=cost_item_inflation_rate.pk)

        self.assertEqual(204, response.status_code)
        self.assertEqual(0, len(CostItemInflationRate.objects.all()))
