...
from scenario.serializers import BalanceSheetSerializer
from scenario.models import BalanceSheet, Scenario
...
class BalanceSheetViewSet(ModelViewSet):
    """ API endpoint for the BalanceSheet model
    """
    ...
    def get_queryset(self):
        ...
        if "username" in query_params:
            ...
            if user_query:
                queryset = BalanceSheet.objects.filter(user=user_query[0],year=0)
            else:
                queryset = None
        ...
        # Filter by parent scenario id
            ...
            if scenario_query:
                queryset = BalanceSheet.objects.filter(scenario=scenario_query[0])
            else:
                queryset = None
    ...
    def specificYear(self, request, pk):
        ...
        try:
            balance_sheets = BalanceSheet.objects.get(year=pk)
