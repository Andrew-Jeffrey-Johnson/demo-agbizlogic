...
from scenario.serializers import CapitalLeaseSerializer
from scenario.models import CapitalLease, Scenario
...
class CapitalLeaseViewSet(ModelViewSet):
    """ API endpoint for the CapitalLease model
    """

    serializer_class = CapitalLeaseSerializer
    queryset = CapitalLease.objects.all()
    ...
    def get_queryset(self):
        ...
        if "username" in query_params:
            ...
            if user_query:
                queryset = CapitalLease.objects.filter(user=user_query[0])
            else:
                queryset = None
        ...
        # Filter by parent scenario id
            ...
            if scenario_query:
                queryset = CapitalLease.objects.filter(scenario=scenario_query[0])
            else:
                queryset = None
    ...
    def specificYear(self, request, pk):
        ...
        try:
            balance_sheets = CapitalLease.objects.get(year=pk)
