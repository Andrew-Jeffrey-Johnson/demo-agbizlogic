...
from scenario.serializers import FutureCapitalLeaseSerializer
from scenario.models import FutureCapitalLease,Scenario
...
class FutureCapitalLeaseViewSet(ModelViewSet):
    """ API endpoint for the FutureCapitalLease model.
    """

    serializer_class = FutureCapitalLeaseSerializer
    queryset = FutureCapitalLease.objects.all()
    ...
    def get_queryset(self):
        ...
        if "scenario" in self.request.query_params:
            try:
                ...
                if scenario_query:
                    queryset = FutureCapitalLease.objects.filter(scenario=scenario_query[0])
                else:
                    queryset = None
